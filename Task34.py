...
def rythm_searcher():
    res = set(''.join([a if (a.lower() not in 'уеёыаоэяию') else '1' for a in ''.join([a if (a.lower(
    ) not in '-бвгджзйклмнпрстфхцчшщ') else '' for a in str(input("Введите вашу кричалку: "))])]).split(' '))
    if len(res) == 1:
        print('Парам пам-пам')
    else:
        print('Пам парам')
